...
public class ProgramAnalysisTest {
  ...
  private void checkIfEqual(Map<JavaRegion, Set<Set<String>>> outputSave,
                            Map<JavaRegion, Set<Set<String>>> outputRead) {
    for (Map.Entry<JavaRegion, Set<Set<String>>> save : outputSave.entrySet()) {
      boolean oneIsEqual = false;
      for (Map.Entry<JavaRegion, Set<Set<String>>> read :
           outputRead.entrySet()) {
        ...
        if (saveRegion.getRegionID().equals(readRegion.getRegionID()) &&
            saveRegion.getStartBytecodeIndex() ==
                readRegion.getStartBytecodeIndex()
            //                        && saveRegion.getEndBytecodeIndex() ==
            //                        readRegion.getEndBytecodeIndex()
            && saveRegion.getRegionPackage().equals(
                   readRegion.getRegionPackage()) &&
  ...
  //        ClassTransformerBaseTest Map<JavaRegion, Set<Set<String>>>
  //        relevantRegionToOptions = new HashMap<>(); Set<String>
  //        relevantOptions = new HashSet<>(); relevantOptions.add("A");
  //        JavaRegion region = new
  //        JavaRegion("ecb89258-9de1-416f-955c-194d09e9b249", Sleep1.PACKAGE,
  //        Sleep1.CLASS, Sleep1.MAIN_METHOD, 20, -1);
  ...
  //        args[1] = "-saveres";
  //
  //        Map<JavaRegion, Set<Set<String>>>  outputSave =
  //        ProgramAnalysis.analyze(Sleep1.CLASS, args, Sleep1.FILENAME,
  //        programFiles, relevantRegionToOptions);
  //
  //        args = new String[0];
  //        Map<JavaRegion, Set<Set<String>>>  outputRead =
  //        ProgramAnalysis.analyze(Sleep1.CLASS, args, Sleep1.FILENAME,
  ...
  public void testAnalyse1() {
    //        Map<JavaRegion, Set<Set<String>>> output =
    //        ProgramAnalysis.analyze(JavaPipeline.LOADTIME_DATABASE,
    //        JavaPipeline.TEST_COLLECTION);
  ...
  public void testReplaceConstraintWithUsedTerms1() {
    ...
    //
    //        String constraint = "(DISTTHRESHOLD = 0 ^ TIMEOUT_Theta ^
    //        !(TIMEOUT_Eta) || !(FREQTHRESHOLD_Beta))";
    //
    //        String res =
    //        ProgramAnalysis.replaceConstraintWithUsedTerms(constraint,
    //        usedTerms); System.out.println(res);
  }
  ...
  public void testReplaceConstraintWithUsedTerms2() {
    ...
    //
    //        String constraint = "(TIMEOUT_Beta ^ TIMEOUT = 0 ^ TIMEOUT_Gamma)
    //        || (TIMEOUT_Beta ^ DISTTHRESHOLD = 0 ^ TIMEOUT_Gamma) ||
    //        (TIMEOUT_Beta ^ FREQTHRESHOLD = 0 ^ TIMEOUT_Gamma) ||
    //        (TIMEOUT_Beta ^ MINALPHA = 0 ^ TIMEOUT_Gamma)";
    //
    //        String res =
    //        ProgramAnalysis.replaceConstraintWithUsedTerms(constraint,
    //        usedTerms); System.out.println(res);
  ...
  public void testReplaceConstraintWithUsedTerms3() {
    ...
    //        usedTerms.add("D");
    //
    //        String constraint = "(!A = 0 ^ !(C = 0) || !(D = 0)) || (!B = 0 ^
    //        !(C = 0) || !(D = 0))";
    //
    //        String res =
    //        ProgramAnalysis.replaceConstraintWithUsedTerms(constraint,
  ...
  public void testGetConjunctions1() {
    ////        String constraint = "(DISTTHRESHOLD = 0 ^ TIMEOUT_Theta ^
    ///!(TIMEOUT_Eta) || !(FREQTHRESHOLD_Beta))";
    //        String constraint = "false || (false || false || true ^ false ||
    //        false || false || !(A = 0) || false)";
    //
  ...
  public void testGetConstraintSet1() {
    ...
    //        usedTerms.add("MINALPHA");
    //
    //        String constraint = "(DISTTHRESHOLD = 0 ^ TIMEOUT ^ !(TIMEOUT) ||
    //        !(FREQTHRESHOLD))";
    //
    //        Set<String> res = ProgramAnalysis.getConstraintSet(constraint,
    //        usedTerms); Assert.assertEquals(res.size(), 3);
  }
  ...
  public void testGetConstraintSet2() {
    ...
    //
    //        Set<String> res = ProgramAnalysis.getConstraintSet(constraint,
    //        usedTerms); Assert.assertEquals(res.size(), 1);
  }
  ...
  public void testGetConstraintSet5() {
    ...
    //
    //        Set<String> res = ProgramAnalysis.getConstraintSet(constraint,
    //        usedTerms); Assert.assertEquals(res.size(), 2);
  }
  ...
  public void testElevator() throws IOException {
    ...
    //        args[1] = "-saveres";
    //
    //        Map<JavaRegion, Set<Set<String>>> partialRegionsToOptions =
    //        ProgramAnalysis.analyze(programName, args,
    //        JavaPipeline.LOADTIME_DATABASE, JavaPipeline.TEST_COLLECTION);
  }
