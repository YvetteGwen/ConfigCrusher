...
public class SPLatDelayPerformanceModelBuilder
    implements PerformanceModelBuilder {
  ...
  public PerformanceModel createModel() {
    Map<Set<String>, Double> configurationToPerformance =
    ...
    for (Map.Entry<Set<String>, Double> entry :
         configurationToPerformance.entrySet()) {
    ...
    Map<Region, Map<Set<String>, Double>> regionToConfigurationPerformance =
        new HashMap<>();
    Region programRegion = new Region("program");
    regionToConfigurationPerformance.put(programRegion,
                                         configurationToPerformance);

    PerformanceModel pm = new SPLatDelayPerformanceModel(
        baseTime, regionToConfigurationPerformance);
    return pm;
  }
  ...
  public PerformanceModel createModel(String[] args) throws IOException {
    ...
    String outputDir = BasePerformanceModelBuilder.DIRECTORY + "/" +
                       this.programName +
  ...
  @Override
  public void writeToFile(PerformanceModel performanceModel)
      throws IOException {
    ObjectMapper mapper = new ObjectMapper();
    String outputFile = BasePerformanceModelBuilder.DIRECTORY + "/" +
                        this.programName +
                        SPLatDelayPerformanceModelBuilder.SPLAT_DIR + "/" +
