...
public class Featurewise extends Approach {
  ...
  public Featurewise(String programName) { super(programName); }
  ...
  public void generateCSVData(Set<PerformanceEntryStatistic> performanceEntries,
    ...
    for (PerformanceEntryStatistic statistic : performanceEntries) {
      ...
      double performance =
          statistic.getRegionsToProcessedPerformanceHumanReadable()
              .values()
              .iterator()
              .next();
    ...
    String outputDir = Evaluation.DIRECTORY + "/" + this.getProgramName() +
                       Approach.DATA_DIR + "/" + Evaluation.FEATURE_WISE +
                       Evaluation.DOT_CSV;
  ...
  public Map<Set<String>, Double> getLearnedModel(List<String> options)
    ...
    String dataDir = Evaluation.DIRECTORY + "/" + this.getProgramName() +
                     Approach.DATA_DIR + "/" + Evaluation.FEATURE_WISE;
    ...
    if (rawTerms.size() != coefs.size()) {
      throw new RuntimeException(
          "The terms and coefs files are not the same length");
    }
    ...
    List<String> significantTerms =
        this.removeTermsWithPValueGreaterThan(rawTerms, pValues, 0.05);
  ...
  public Set<PerformanceEntryStatistic>
  getFeaturewiseEntries(Set<PerformanceEntryStatistic> performanceEntries) {
    Set<Set<String>> configurations =
        this.getConfigurations(performanceEntries);
    Set<Set<String>> featurewiseConfigurations;

    if (this.getFm() == null) {
      featurewiseConfigurations =
          Featurewise.getFeaturewiseConfigurations(configurations);

    } else {
      featurewiseConfigurations = Featurewise.getFeaturewiseConfigurations(
          configurations, this.getFm());
    }
  ...
  getFeaturewiseConfigurations(Set<Set<String>> configurations,
                               FeatureModel fm) {
  ...
  public static Set<Set<String>>
  getFeaturewiseConfigurations(Collection<String> options) {
    Set<Set<String>> featurewiseConfigurations = new HashSet<>();
  ...
  getConfigurations(Set<PerformanceEntryStatistic> performanceEntryStatistics) {
