...
public class SPLatDelay extends Approach {
  ...
  public SPLatDelay(String programName) { super(programName); }
  ...
  public List<Coverage> readFileCoverage() throws IOException {
    String coverageFile = SPLatDelayMain.DIRECTORY + "/" +
                          this.getProgramName() + "/coverage" +
                          Options.DOT_JSON;
    File file = new File(coverageFile);

    ObjectMapper mapper = new ObjectMapper();
    List<Coverage> results =
        mapper.readValue(file, new TypeReference<List<Coverage>>() {});

    return results;
  }

  public Set<PerformanceEntryStatistic>
  getSPLatEntries(Set<PerformanceEntryStatistic> performanceEntries) {
    //        Set<Set<String>> configurations =
    //        this.getConfigurations(performanceEntries); Set<Set<String>>
    //        splatConfigurations =
    //        this.getSPLatDelayConfigurations(configurations);
    Set<Set<String>> splatDelayConfigurations =
        this.getSPLatDelayConfigurations();
