...
public class VariableBeforeReturnTransformer extends BaseMethodTransformer {
  ...
  public Set<MethodNode> getMethodsToInstrument(ClassNode classNode) {
    ...
    for (MethodNode methodNode : classNode.methods) {
      ...
      while (instructionsIterator.hasNext()) {
        ...
        //                        if(previousInstruction.getOpcode() >=
        //                        Opcodes.INVOKEVIRTUAL &&
        //                        previousInstruction.getOpcode() <=
        ...
        if (!methodNode.name.equals("<init>") &&
            !methodInstruction.owner.equals("java/lang/Object")) {
      ...
      if (instruction.getOpcode() >= Opcodes.IRETURN &&
        ...
        if (previousInstruction.getType() == AbstractInsnNode.METHOD_INSN) {
          MethodInsnNode methodInstruction =
              (MethodInsnNode)previousInstruction;

          if (!methodNode.name.equals("<init>") &&
              !methodInstruction.owner.equals("java/lang/Object")) {
            InsnList localVarInsts = this.addInstructionsBeforeReturn(
                instruction, methodNode.maxLocals);
  ...
  private InsnList addInstructionsBeforeReturn(AbstractInsnNode returnInst,
                                               int maxLocals) {
    InsnList newInsts = new InsnList();
    ...
    if (returnInst.getOpcode() == Opcodes.IRETURN) {
      ...
      throw new IllegalArgumentException(
          "The instruction node is not a return");
    }
