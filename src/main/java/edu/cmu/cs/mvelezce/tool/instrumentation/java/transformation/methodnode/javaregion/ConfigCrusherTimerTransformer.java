...
public class ConfigCrusherTimerTransformer
    extends ConfigCrusherRegionTransformer {

  private Set<JavaRegion> instrumentedRegionsInSameBlock = new HashSet<>();

  public ConfigCrusherTimerTransformer(
  ...
  public void transformMethod(MethodNode methodNode) {
    ...
    InsnList newInstructions =
        this.instrumentRegion(methodNode, regionsInMethod);
  ...
  private InsnList instrumentStart(MethodBlock methodBlock,
  ...
  private InsnList instrumentEnd(MethodBlock methodBlock,
                                 List<JavaRegion> regionsInMethod) {
    InsnList newInstructions = new InsnList();
  ...
  public InsnList getInstructionsStartRegion(JavaRegion javaRegion) {
    ...
    instructionsStartRegion.add(
        new MethodInsnNode(Opcodes.INVOKESTATIC,
                           "edu/cmu/cs/mvelezce/tool/analysis/region/Regions",
                           "enter", "(Ljava/lang/String;)V", false));
    //        instructionsStartRegion.add(new
    //        MethodInsnNode(Opcodes.INVOKESTATIC,
    //        "edu/cmu/cs/mvelezce/tool/analysis/region/RegionsCounter",
    //        "enter", "(Ljava/lang/String;)V", false));
  ...
  public InsnList getInstructionsEndRegion(JavaRegion javaRegion) {
    ...
    //        instructionsEndRegion.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
    //        "edu/cmu/cs/mvelezce/tool/analysis/region/RegionsCounter", "exit",
   ...
   */
  private InsnList instrumentRegion(MethodNode methodNode,
                                    List<JavaRegion> regionsInMethod) {
    System.out.println("########### " +
                       this.getMethodNodeToClassNode().get(methodNode).name +
                       " " + methodNode.name);

    InsnList newInstructions =
        this.instrumentRegionsStartAndEndSameBlock(methodNode, regionsInMethod);
    methodNode.instructions.clear();
    methodNode.instructions.add(newInstructions);
  ...
  private InsnList
  instrumentRegionsStartAndEndSameBlock(MethodNode methodNode,
                                        List<JavaRegion> regionsInMethod) {
    InsnList newInstructions = new InsnList();
    ListIterator<AbstractInsnNode> iterator =
        methodNode.instructions.iterator();

    // Have to loop through the instructions to avoid changing the current
    // instructions in the method node
    ...
    ListIterator<AbstractInsnNode> instructionsIterator =
  ...
  private InsnList instrumentNormal(MethodNode methodNode,
                                    List<JavaRegion> regionsInMethod) {
    List<JavaRegion> regionsInMethodReversed = new ArrayList<>(regionsInMethod);
    Collections.reverse(regionsInMethodReversed);
      ...
      if (this.getEndRegionBlocksWithReturn().contains(block)) {
        if (!block.isWithReturn()) {
          throw new RuntimeException(
              "The analysis stated that the block had a return, but the graph says "
              +
        ...
        while ((opcode < Opcodes.IRETURN || opcode > Opcodes.RETURN) &&
               opcode != Opcodes.RET) {
          newInstructions.add(instruction);
          instruction = instructionsIterator.next();
        ...
        InsnList endInstructions =
            this.instrumentEnd(block, regionsInMethodReversed);
        ...
        InsnList endInstructions =
            this.instrumentEnd(block, regionsInMethodReversed);
        newInstructions.add(endInstructions);
        InsnList startInstructions =
            this.instrumentStart(block, regionsInMethod);
  ...
  private boolean startAndEndInSameBlock(JavaRegion region, MethodBlock block) {
    ...
    if (region.getStartMethodBlock() !=
        region.getEndMethodBlocks().iterator().next()) {
      return false;
    }
   ...
   * @param methodNode
   */
  private InsnList instrumentEntireMethod(MethodNode methodNode,
                                          JavaRegion region) {
    InsnList newInstructions = new InsnList();
    ListIterator<AbstractInsnNode> iterator =
        methodNode.instructions.iterator();
    ...
    for (MethodBlock endBlock : endMethodBlocks) {
      ...
      AbstractInsnNode lastInstruction =
          blockInstructions.get(blockInstructions.size() - 1);
      int opcodeLastInstruction = lastInstruction.getOpcode();

      if ((opcodeLastInstruction < Opcodes.IRETURN ||
           opcodeLastInstruction > Opcodes.RETURN) &&
          opcodeLastInstruction != Opcodes.RET &&
          opcodeLastInstruction != Opcodes.ATHROW) {
        ...
        if ((opcodeLastInstruction < Opcodes.IRETURN ||
          ...
          if (!this.getMethodsToGraphs().get(methodNode).isWithWhileTrue()) {
            throw new RuntimeException(
                "The last instruction in a method with return is not a return instruction");
          }
        }
      }
