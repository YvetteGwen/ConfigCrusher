...
public class SimpleCompression extends BaseCompression {
  ...
  public Set<Set<String>> compressConfigurations() {
    ...
    Map<Set<String>, Set<Set<String>>> optionsToConfigurationsToExecute =
        new HashMap<>();

    for (Set<String> options : filteredOptions) {
      Set<Set<String>> configurationsToExecuteForOption = new HashSet<>(
          edu.cmu.cs.mvelezce.tool.Helper.getConfigurations(options));
      optionsToConfigurationsToExecute.put(options,
                                           configurationsToExecuteForOption);
    }
    ...
    if (optionsToConfigurationsToExecute.size() == 1) {
      configurationsToExecute.addAll(optionsToConfigurationsToExecute.entrySet()
                                         .iterator()
                                         .next()
                                         .getValue());
      return configurationsToExecute;
    ...
    Iterator<Map.Entry<Set<String>, Set<Set<String>>>>
        optionsToConfigurationsToExecuteIterator =
            optionsToConfigurationsToExecute.entrySet().iterator();
    Map.Entry<Set<String>, Set<Set<String>>> entry1 =
        optionsToConfigurationsToExecuteIterator.next();

    while (optionsToConfigurationsToExecuteIterator.hasNext()) {
      Map.Entry<Set<String>, Set<Set<String>>> entry2 =
          optionsToConfigurationsToExecuteIterator.next();
      ...
      if (entry1.getValue().size() <= entry2.getValue().size()) {
        this.simpleMerging(entry1, entry2, pivotOptions,
                           configurationsToExecute);
      } else {
        this.simpleMerging(entry2, entry1, pivotOptions,
                           configurationsToExecute);
      }
  ...
  private void
  simpleMerging(Map.Entry<Set<String>, Set<Set<String>>> largeEntry,
                Map.Entry<Set<String>, Set<Set<String>>> smallEntry,
                Set<String> pivotOptions,
                Set<Set<String>> configurationsToExecute) {
    Map<Set<String>, List<Set<String>>> largePivotToConfigs =
        this.pivotValueToConfigs(pivotOptions, largeEntry.getValue());
    Map<Set<String>, List<Set<String>>> smallPivotToConfigs =
        this.pivotValueToConfigs(pivotOptions, smallEntry.getValue());
  ...
  private Map<Set<String>, List<Set<String>>>
  pivotValueToConfigs(Set<String> pivot, Set<Set<String>> configs) {
