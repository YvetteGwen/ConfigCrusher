...
/**
 * A Control Flow Graph builder. It implements the Visitor pattern to add each
...
public class CFGBuilder extends ReturnerVisitor {
  ...
  public Expression visitConstantConfigurationExpression(
    ...
    if (expressionConfigurationConstant == null) {
      throw new IllegalArgumentException(
          "The expressionConfigurationConstant cannot be null");
    }
  ...
  visitVariableExpression(VariableExpression expressionVariable) {
    if (expressionVariable == null) {
      throw new IllegalArgumentException(
          "The expressionVariable cannot be null");
    }
  ...
  visitAssignmentStatement(AssignmentStatement statementAssignment) {
    if (statementAssignment == null) {
      throw new IllegalArgumentException(
          "The statementAssignment cannot be null");
    }
  ...
  /**
   * This method pushes this IfStatement to a stack that and analyzes the Then
   * branch. This is done so that this BasicBlock also has an edge to the next
   * BasicBlock after then Then branch.
   *
   * @param statementIf
   */
  @Override
  ...
  /**
   * Checks if there the builder came from analyzing a IfStatement. This allows
   * to create an edge between the IfStatement and the BasicBlock after it.
   *
   * @param basicBlock
   */
  ...
  /**
   * Adds the conditions that affect this statement. It does not know the exact
   * value of the expression; rather it knows what the expression(s) are.
   *
   * @param statement
   * @return
   */
  private BasicBlock checkConditions(Statement statement) {
    ...
    if (this.conditionStack.isEmpty()) {
      ...
      // TODO should we reverse this list? If it is reverse, we know what is the
      // first condition for this statement, specially in nested ifs
