...
public abstract class BaseRegionInstrumenter extends BaseInstrumenter {
  ...
  public void instrument(String[] args)
    ...
    File outputFile = new File(BaseRegionInstrumenter.DIRECTORY + "/" +
                               this.getProgramName());
    Options.checkIfDeleteResult(outputFile);

    if (outputFile.exists()) {
      Collection<File> files =
          FileUtils.listFiles(outputFile, new String[] {"json"}, false);
  ...
  writeToFile(Map<JavaRegion, Set<Set<String>>> relevantRegionsToOptions)
    ...
    String outputFile = BaseRegionInstrumenter.DIRECTORY + "/" +
                        this.getProgramName() + "/" + this.getProgramName() +
                        Options.DOT_JSON;
    ...
    for (Map.Entry<JavaRegion, Set<Set<String>>> regionToOptionsSet :
      ...
      JavaRegion newRegion = new JavaRegion(
          oldRegion.getRegionID(), oldRegion.getRegionPackage(),
          oldRegion.getRegionClass(), oldRegion.getRegionMethod(),
          oldRegion.getStartBytecodeIndex(),
          oldRegion.getStartMethodBlock().getID(), endBlocksIDs);
      DecisionAndOptions decisionAndOptions =
          new DecisionAndOptions(newRegion, regionToOptionsSet.getValue());
