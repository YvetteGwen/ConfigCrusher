...
public abstract class BaseExecutor implements Executor {
  ...
  public BaseExecutor(String programName, String entryPoint, String classDir,
  ...
  protected Set<PerformanceEntryStatistic>
  averageExecutions(List<Set<DefaultPerformanceEntry>> performanceEntriesList) {
    Set<DefaultPerformanceEntry> performanceEntrySet =
        performanceEntriesList.iterator().next();
    ...
    // Get results
    for (DefaultPerformanceEntry performanceEntry : performanceEntrySet) {
      PerformanceEntryStatistic entryStatistic = new PerformanceEntryStatistic(
          true, performanceEntry.getConfiguration());
    ...
    for (Set<String> configuration : configurations) {
      ...
      for (Set<DefaultPerformanceEntry> performanceEntries :
    ...
    for (PerformanceEntryStatistic performanceEntryStatistic : results) {
      ...
      // Mean
      performanceEntryStatistic.setRegionsToRawPerformance(
          performanceEntryStatistic.getRegionsRawMean());
      performanceEntryStatistic.setRegionsToRawPerformanceHumanReadable(
          performanceEntryStatistic.getRegionsRawMeanHumanReadable());
      performanceEntryStatistic.setRegionsToProcessedPerformance(
          performanceEntryStatistic.getRegionsProcessedMean());
      performanceEntryStatistic.setRegionsToProcessedPerformanceHumanReadable(
          performanceEntryStatistic.getRegionsMeanProcessedHumanReadable());
    }
  ...
  @Override
  public Set<PerformanceEntryStatistic> execute(String[] args)
    ...
    if (root.exists()) {
      List<Set<DefaultPerformanceEntry>> performanceEntriesList =
          new ArrayList<>();
      ...
      while (outputFile.exists()) {
        Set<DefaultPerformanceEntry> results =
  ...
  @Override
  public Set<PerformanceEntryStatistic> execute()
      throws IOException, InterruptedException {
    List<Set<DefaultPerformanceEntry>> performanceEntriesList =
        new ArrayList<>();
  ...
  public Set<Set<String>> getConfigurations() { return configurations; }

  public int getRepetitions() { return repetitions; }

  protected Set<DefaultPerformanceEntry> aggregateExecutions(File outputFile)
      throws IOException {
    Collection<File> files =
        FileUtils.listFiles(outputFile, new String[] {"json"}, true);
  ...
  public Set<String> getOptions(File file) throws IOException {
    ...
    Set<String> options =
        mapper.readValue(file, new TypeReference<Set<String>>() {});

    return options;
  }

  @Override
  public void writeToFile(String iteration, Set<String> configuration,
                          Map<String, Long> regionsToProcessedPerformance)
      throws IOException {
    ObjectMapper mapper = new ObjectMapper();
    String outputFile = this.getOutputDir() + "/" + this.programName + "/" +
                        iteration + "/" + UUID.randomUUID() + Options.DOT_JSON;
    File file = new File(outputFile);
    file.getParentFile().mkdirs();

    Execution execution =
        new Execution(configuration, regionsToProcessedPerformance);
  ...
  //    @Override
  //    public void writeToFile(String iteration, Set<String> configuration,
  //    List<Region> executedRegions) throws IOException {
  //        ObjectMapper mapper = new ObjectMapper();
  //        String outputFile = BaseExecutor.DIRECTORY + "/" + this.programName
  //        + "/" + iteration + "/" + UUID.randomUUID()
  //                + Options.DOT_JSON;
  //        File file = new File(outputFile);
  //        file.getParentFile().mkdirs();
  //
  ...
  public DefaultPerformanceEntry readFromFile(File file) throws IOException {
    ObjectMapper mapper = new ObjectMapper();
    Execution execution =
        mapper.readValue(file, new TypeReference<Execution>() {});

    //        execution.checkTrace();
    DefaultPerformanceEntry performanceEntry =
        new DefaultPerformanceEntry(execution);
