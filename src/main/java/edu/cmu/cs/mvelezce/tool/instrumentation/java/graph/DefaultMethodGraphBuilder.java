...
public class DefaultMethodGraphBuilder extends BaseMethodGraphBuilder {
  ...
  public DefaultMethodGraphBuilder(MethodNode methodNode) { super(methodNode); }
  ...
  public void addEdges() {
    ...
    while (instructionsIterator.hasNext()) {
      ...
      if (possibleBlock != null) {
        ...
        // This can happen when the possible block is the target of a jump
        // instruction
      ...
      if (type == AbstractInsnNode.JUMP_INSN) {
        ...
        MethodBlock destinationBlock =
            this.getGraph().getMethodBlock(jumpInsn.label);
        ...
        for (TryCatchBlockNode tryCatchBlock :
             this.getMethodNode().tryCatchBlocks) {
          ...
          while (insnNode != endNode && insnNode.getNext() != endNode) {
            ...
            if (insnNode == instruction) {
              MethodBlock destinationBlock =
                  this.getGraph().getMethodBlock(tryCatchBlock.handler);
        ...
        if (type == AbstractInsnNode.TABLESWITCH_INSN) {
          ...
          MethodBlock destinationBlock =
              this.getGraph().getMethodBlock(tableSwitchInsn.dflt);
          this.getGraph().addEdge(block, destinationBlock);
          ...
          MethodBlock destinationBlock =
              this.getGraph().getMethodBlock(lookupSwitchInsn.dflt);
          this.getGraph().addEdge(block, destinationBlock);
  ...
  public void addInstructions() {
    ...
    // TODO do not hard code 3. This can happen if the method has a while(true)
    // loop. Then there is no return
  ...
  public void getBlocks() {
    ...
    while (instructionsIterator.hasNext()) {
      ...
      if (type == AbstractInsnNode.JUMP_INSN) {
        ...
        // This is a return in the middle of the body and there is a block that
        // was already created for the next instruction
        if (block != null) {
          continue;
        }
