...
public class TaintAnalysis {
  ...
  /**
   * Perform a taint analysis in the CFG and a map that represents, for every
   * simpleMerging block, the variables that might be tainted.
   *
   * @param cfg
   * @return
   */
  public static Map<BasicBlock, Set<PossibleTaint>> analyze(CFG cfg) {
    if (cfg.getSuccessors(cfg.getEntry()).size() > 1) {
      throw new IllegalArgumentException(
          "The entry point of the CFG has more than 1 edge");
    }

    // Initialize all basic bloks to an empty set
    Map<BasicBlock, Set<PossibleTaint>> instructionsToPossibleTaints =
        new LinkedHashMap<>();
    ...
    while (!worklist.isEmpty()) {
      ...
      if (predecessors.size() > 2) {
        throw new IllegalArgumentException(
            "We do not support switch statements yet");
      }
      ...
      if (predecessors.size() == 1) {
        possibleTaintsBefore =
            instructionsToPossibleTaints.get(predecessors.get(0));
      } else if (predecessors.size() == 2) {
        possibleTaintsBefore = TaintAnalysis.join(
            instructionsToPossibleTaints.get(predecessors.get(0)),
            instructionsToPossibleTaints.get(predecessors.get(1)));
      }
      possibleTaintsBefore = TaintAnalysis.join(
          instructionsToPossibleTaints.get(instruction), possibleTaintsBefore);

      // Calculate new state and save
      Set<PossibleTaint> possibleTaintsAfter =
          TaintAnalysis.transfer(instruction, possibleTaintsBefore);
      instructionsToPossibleTaints.put(instruction, possibleTaintsAfter);
      // TODO this only works because the we do not currently support cases were
      // the state changes in the last instruction of a block that we need to
      // time ie sleep(x=A)
  ...
  public static Set<PossibleTaint>
  transfer(BasicBlock instruction, Set<PossibleTaint> oldPossibleTaints) {
    TransferVisitor transferVisitor =
        new TransferVisitor(oldPossibleTaints, instruction.getConditions());
    instruction.getStatement().accept(transferVisitor);

    Set<PossibleTaint> newPossibleTaints =
        transferVisitor.getPossibleTaintedVariables();
    Set<PossibleTaint> killedPossibleTaints =
        transferVisitor.getKilledPossibleTaintedVariables();
  ...
  public static Set<PossibleTaint> join(Set<PossibleTaint> possibleTaintsA,
                                        Set<PossibleTaint> possibleTaintsB) {
    Set<PossibleTaint> result = new HashSet<>();
    Map<VariableExpression, Set<ConstantConfigurationExpression>>
        possibleTaintedVariablesB = new HashMap<>();

    for (PossibleTaint possibleTaintB : possibleTaintsB) {
      possibleTaintedVariablesB.put(possibleTaintB.getVariable(),
                                    possibleTaintB.getConfigurations());
    }

    for (PossibleTaint possibleTaintA : possibleTaintsA) {
      Set<ConstantConfigurationExpression> hold =
          new HashSet<>(possibleTaintA.getConfigurations());
    ...
    for (Map.Entry<VariableExpression, Set<ConstantConfigurationExpression>>
             entry : possibleTaintedVariablesB.entrySet()) {
  ...
  private static class TransferVisitor extends ReturnerVisitor {
    ...
    public TransferVisitor(Set<PossibleTaint> oldPossibleTaints,
    ...
    /**
     * Signals that this is a configuration option an that it is possibly
     * tainting something.
     *
    ...
    public Expression visitConstantConfigurationExpression(
        ConstantConfigurationExpression expressionConfigurationConstant) {
      Expression expression = super.visitConstantConfigurationExpression(
          expressionConfigurationConstant);
      this.tainting = true;
      ...
      if (this.mergingOptions) {
        for (PossibleTaint taintedVariable : this.possibleTaintedVariables) {
          taintedVariable.getConfigurations().add(
              expressionConfigurationConstant);
        }
      }
    ...
    @Override
    public Expression
    visitVariableExpression(VariableExpression expressionVariable) {
      ...
      if (this.inAssignment) {
        for (PossibleTaint taintedVariable : this.oldPossibleTaints) {
          if (taintedVariable.getVariable().equals(expressionVariable)) {
            this.tainting = true;
            this.possibleTaintingConfigurations.addAll(
                taintedVariable.getConfigurations());
            // Since we are checking for the variable, we can break, since there
            // is only one variable in the assignment
            break;
          }
        }
      }
        ...
        for (PossibleTaint oldTaintedVariable : this.oldPossibleTaints) {
          if (this.conditions.contains(oldTaintedVariable.getVariable())) {
            if (this.possibleTaintedVariables.isEmpty()) {
              Set<ConstantConfigurationExpression> implicitConfigurations =
                  new HashSet<>(oldTaintedVariable.getConfigurations());
              this.possibleTaintedVariables.add(new PossibleTaint(
                  expressionVariable, implicitConfigurations));
            } else {
              for (PossibleTaint taintedVariable :
                   this.possibleTaintedVariables) {
                if (taintedVariable.getVariable().equals(expressionVariable)) {
                  taintedVariable.getConfigurations().addAll(
                      oldTaintedVariable.getConfigurations());
                  // Since we are checking for the variable, we can break, since
                  // there is only one variable in the assignment
                  break;
                }
              }
            }
      ...
      if (this.inRelevantStatement &&
          !this.possibleTaintedVariables.isEmpty()) {
        for (PossibleTaint oldPossibleTaint : this.oldPossibleTaints) {
          if (!this.possibleTaintedVariables.contains(oldPossibleTaint) &&
              oldPossibleTaint.getVariable().equals(expressionVariable)) {
            ...
            for (PossibleTaint possibleTaint : this.possibleTaintedVariables) {
              if (possibleTaint.getVariable().equals(expressionVariable)) {
                possibleTaint.getConfigurations().addAll(
                    oldPossibleTaint.getConfigurations());
                // Since we are checking for the variable, we can break, since
                // there is only one variable in the assignment
                break;
              }
    ...
    /**
     * Statement where tainting can occur. It can be x = X, x = a (where a is
     * possibly tainted), and all previous cases and implicit flow by being
     * inside a relevant statement
     *
     * @param statementAssignment
     * @return
     */
    public Void
    visitAssignmentStatement(AssignmentStatement statementAssignment) {
      this.inAssignment = true;
      // Right side option or right side possibly tainted
      statementAssignment.getRight().accept(this);

      if (this.tainting) {
        Set<ConstantConfigurationExpression> configurations =
            new HashSet<>(this.possibleTaintingConfigurations);
        this.possibleTaintedVariables.add(new PossibleTaint(
            statementAssignment.getVariable(), configurations));
      }
      ...
      for (PossibleTaint taintedVariable : this.oldPossibleTaints) {
        if (!this.possibleTaintedVariables.contains(taintedVariable) &&
            taintedVariable.getVariable().equals(
                statementAssignment.getVariable())) {
    ...
    /**
     * Relevant statement that can be inside a region and its condition could be
     * tainted.
     *
     * @param statementIf
     * @return
  ...
  /**
   * A possible tainted variable and a set of configurations that might have
   * tainted it.
   */
  public static class PossibleTaint {
    ...
    public PossibleTaint(VariableExpression variable,
                         Set<ConstantConfigurationExpression> configurations) {
      if (variable == null) {
