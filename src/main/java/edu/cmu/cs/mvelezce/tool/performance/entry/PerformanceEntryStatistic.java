...
public class PerformanceEntryStatistic extends DefaultPerformanceEntry {
  ...
  private Map<Region, List<Double>> regionsToRawMinMaxHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, Double> regionsToRawMeanHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, Double> regionsToRawStdHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, List<Double>> regionsToRawCIHumanReadable =
      new LinkedHashMap<>();

  private Map<Region, List<Long>> regionsToProcessedMinMax =
      new LinkedHashMap<>();
  ...
  private Map<Region, List<Double>> regionsToProcessedCI =
      new LinkedHashMap<>();

  private Map<Region, List<Double>> regionsToProcessedMinMaxHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, Double> regionsToProcessedMeanHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, Double> regionsToProcessedStdHumanReadable =
      new LinkedHashMap<>();
  private Map<Region, List<Double>> regionsToProcessedCIHumanReadable =
      new LinkedHashMap<>();

  public PerformanceEntryStatistic(boolean measured,
  ...
  /**
   * The assumption is that the values in the map are in nanoseconds and we
   * transform them to seconds
   *
   * @param regionsToPerformance
   * @return
   */
  public static Map<Region, Double>
  stdToHumanReadable(Map<Region, Double> regionsToPerformance) {
    Map<Region, Double> result = new LinkedHashMap<>();

    for (Map.Entry<Region, Double> entry : regionsToPerformance.entrySet()) {
      result.put(entry.getKey(),
                 PerformanceEntryStatistic.toSeconds(entry.getValue()));
    }

    return result;
  }

  public static Map<Region, List<Double>>
  ciToHumanReadable(Map<Region, List<Double>> regionsToPerformance) {
    Map<Region, List<Double>> result = new LinkedHashMap<>();

    for (Map.Entry<Region, List<Double>> entry :
         regionsToPerformance.entrySet()) {
  ...
  private void
  calculateRawMinMax(int index,
                     List<DefaultPerformanceEntry> performanceEntries) {
    for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
      Iterator<Map.Entry<Region, Long>> performanceEntryIterator =
          performanceEntry.getRegionsToRawPerformance().entrySet().iterator();
      Iterator<Map.Entry<Region, List<Long>>> regionsToRawMinMaxMeanIterator =
          this.regionsToRawMinMax.entrySet().iterator();

      while (performanceEntryIterator.hasNext() &&
             regionsToRawMinMaxMeanIterator.hasNext()) {
        Map.Entry<Region, Long> entry1 = performanceEntryIterator.next();
        Map.Entry<Region, List<Long>> entry2 =
            regionsToRawMinMaxMeanIterator.next();

        if (!entry1.getKey().getRegionID().equals(
                entry2.getKey().getRegionID())) {
      ...
      if (performanceEntryIterator.hasNext() &&
          !regionsToRawMinMaxMeanIterator.hasNext()) {
        throw new RuntimeException(
            "There were items left in the regions iterator");
      }

      if (!performanceEntryIterator.hasNext() &&
          regionsToRawMinMaxMeanIterator.hasNext()) {
        throw new RuntimeException(
            "There were items left in the performance iterator");
      }
    }
  }

  private void
  calculateProcessedMinMax(int index,
                           List<DefaultPerformanceEntry> performanceEntries) {
    for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
      Iterator<Map.Entry<Region, Long>> performanceEntryIterator =
          performanceEntry.getRegionsToProcessedPerformance()
              .entrySet()
              .iterator();
      Iterator<Map.Entry<Region, List<Long>>>
          regionsToProcessedMinMaxMeanIterator =
              this.regionsToProcessedMinMax.entrySet().iterator();

      while (performanceEntryIterator.hasNext() &&
             regionsToProcessedMinMaxMeanIterator.hasNext()) {
        Map.Entry<Region, Long> entry1 = performanceEntryIterator.next();
        Map.Entry<Region, List<Long>> entry2 =
            regionsToProcessedMinMaxMeanIterator.next();
  ...
  public void
  calculateMinMax(List<DefaultPerformanceEntry> performanceEntries) {
    // Raw data
    for (Region region : performanceEntries.iterator()
                             .next()
                             .getRegionsToRawPerformance()
                             .keySet()) {
    ...
    for (Map.Entry<Region, List<Long>> entry :
         this.regionsToRawMinMax.entrySet()) {
    ...
    for (Region region : performanceEntries.iterator()
                             .next()
                             .getRegionsToProcessedPerformance()
                             .keySet()) {
      List<Long> minMax = new ArrayList<>();
      minMax.add(Long.MAX_VALUE);
      minMax.add(Long.MIN_VALUE);
    ...
    for (Map.Entry<Region, List<Long>> entry :
         this.regionsToProcessedMinMax.entrySet()) {
      List<Double> minMax = new ArrayList<>();
  ...
  public void calculateMean(List<DefaultPerformanceEntry> performanceEntries) {
    ...
    for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
      Iterator<Map.Entry<Region, Long>> performanceEntryIterator =
          performanceEntry.getRegionsToRawPerformance().entrySet().iterator();
      Iterator<Map.Entry<Region, Long>> regionsToMeanIterator =
          this.regionsToRawMean.entrySet().iterator();

      while (performanceEntryIterator.hasNext() &&
             regionsToMeanIterator.hasNext()) {
        Map.Entry<Region, Long> entry1 = performanceEntryIterator.next();
      ...
      if (performanceEntryIterator.hasNext() &&
          !regionsToMeanIterator.hasNext()) {
        throw new RuntimeException(
            "There were items left in the regions to mean iterator");
      }

      if (!performanceEntryIterator.hasNext() &&
          regionsToMeanIterator.hasNext()) {
        throw new RuntimeException(
            "There were items left in the performance entry iterator");
      }
    }

    for (Map.Entry<Region, Long> regionToMean :
         this.regionsToRawMean.entrySet()) {
    ...
    for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
      Iterator<Map.Entry<Region, Long>> performanceEntryIterator =
          performanceEntry.getRegionsToProcessedPerformance()
              ...
              .iterator();
      Iterator<Map.Entry<Region, Long>> regionsToMeanIterator =
          this.regionsToProcessedMean.entrySet().iterator();
    ...
    for (Map.Entry<Region, Long> regionToMean :
         this.regionsToProcessedMean.entrySet()) {
      long sum = regionToMean.getValue();
      sum /= performanceEntries.size();
      this.regionsToProcessedMean.put(regionToMean.getKey(), sum);
    }

    this.regionsToRawMeanHumanReadable =
        PerformanceEntryStatistic.toHumanReadable(this.regionsToRawMean);
    this.regionsToProcessedMeanHumanReadable =
        PerformanceEntryStatistic.toHumanReadable(this.regionsToProcessedMean);
  }
  ...
  public void calculateStd(List<DefaultPerformanceEntry> performanceEntries) {
    ...
    for (Map.Entry<Region, Double> regionToRawStd :
         this.regionsToRawStd.entrySet()) {
      Region region = regionToRawStd.getKey();
      List<Double> rawEntries = new ArrayList<>();

      for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
        for (Map.Entry<Region, Long> regionToRaw :
             performanceEntry.getRegionsToRawPerformance().entrySet()) {
          if (!region.getRegionID().equals(
                  regionToRaw.getKey().getRegionID())) {
            continue;
          }
    ...
    for (Map.Entry<Region, Double> regionToProcessedStd :
         this.regionsToProcessedStd.entrySet()) {
      Region region = regionToProcessedStd.getKey();
      List<Double> rawEntries = new ArrayList<>();

      for (DefaultPerformanceEntry performanceEntry : performanceEntries) {
        for (Map.Entry<Region, Long> regionToProcessed :
             performanceEntry.getRegionsToProcessedPerformance().entrySet()) {
          if (!region.getRegionID().equals(
                  regionToProcessed.getKey().getRegionID())) {
            continue;
          }
    ...
    this.regionsToRawStdHumanReadable =
        PerformanceEntryStatistic.stdToHumanReadable(this.regionsToRawStd);
    this.regionsToProcessedStdHumanReadable =
        PerformanceEntryStatistic.stdToHumanReadable(
            this.regionsToProcessedStd);
  }
  ...
  public void calculateCIRaw(List<DefaultPerformanceEntry> performanceEntries) {
    ...
    for (Map.Entry<Region, List<Double>> regionToRawCI :
         this.regionsToRawCI.entrySet()) {
  ...
  public void
  calculateCIProcessed(List<DefaultPerformanceEntry> performanceEntries) {
    // Process data
    for (Region region : performanceEntries.iterator()
                             .next()
                             .getRegionsToProcessedPerformance()
    ...
    for (Map.Entry<Region, List<Double>> regionToProcessedCI :
         this.regionsToProcessedCI.entrySet()) {
  ...
  public void calculateCI(List<DefaultPerformanceEntry> performanceEntries) {
    ...
    this.regionsToRawCIHumanReadable =
        PerformanceEntryStatistic.ciToHumanReadable(this.regionsToRawCI);
    this.regionsToProcessedCIHumanReadable =
        PerformanceEntryStatistic.ciToHumanReadable(this.regionsToProcessedCI);
  }
  ...
  public Map<Region, Double> getRegionsRawMinHumanReadable() {
    ...
    for (Map.Entry<Region, List<Double>> entry :
         this.regionsToRawMinMaxHumanReadable.entrySet()) {
      result.put(entry.getKey(), entry.getValue().get(0));
    }
  ...
  public Map<Region, Double> getRegionsMinProcessedHumanReadable() {
    ...
    for (Map.Entry<Region, List<Double>> entry :
         this.regionsToProcessedMinMaxHumanReadable.entrySet()) {
      result.put(entry.getKey(), entry.getValue().get(0));
    }
  ...
  public Map<Region, Double> getRegionsRawMeanHumanReadable() {
    ...
    for (Map.Entry<Region, Double> entry :
         this.regionsToRawMeanHumanReadable.entrySet()) {
      result.put(entry.getKey(), entry.getValue());
    }
  ...
  public Map<Region, Long> getRegionsProcessedMean() {
    ...
    for (Map.Entry<Region, Long> entry :
         this.regionsToProcessedMean.entrySet()) {
      result.put(entry.getKey(), entry.getValue());
    }
  ...
  public Map<Region, Double> getRegionsMeanProcessedHumanReadable() {
    ...
    for (Map.Entry<Region, Double> entry :
         this.regionsToProcessedMeanHumanReadable.entrySet()) {
      result.put(entry.getKey(), entry.getValue());
    }
  ...
  public Map<Region, Long> getRegionsToRawMean() { return regionsToRawMean; }

  public Map<Region, Double> getRegionsToRawStd() { return regionsToRawStd; }
