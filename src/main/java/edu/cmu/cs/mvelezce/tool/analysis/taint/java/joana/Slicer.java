...
public class Slicer {
  ...
  //    public static Map<JavaRegion, Set<String>> analyze(String file,
  //    Map<String, String> featuresToLabels) throws IOException {
  //        Map<JavaRegion, Set<String>> relevantRegionToOptions = new
  //        HashMap<>(); SDG sdg = SDG.readFrom(file, new
  //        SecurityNode.SecurityNodeFactory()); Set<SDGNode> nodes =
  //        sdg.vertexSet();
  //
  //        for(Map.Entry<String, String> featureToLabel :
  //        featuresToLabels.entrySet()) {
  //            System.out.println("################################# " +
  //            featureToLabel.getKey());
  //
  ...
  ////                if(node.getLabel().startsWith("if ") &&
  ///!node.getBytecodeMethod().startsWith("java.")) { / relevantNodes.add(node);
  ////                }
  ...
  //                String entryMethod =
  //                entryPackage.substring(entryPackage.lastIndexOf(".") + 1);
  //                entryPackage = entryPackage.replace("." + entryMethod, "");
  //                String entryClass =
  //                entryPackage.substring(entryPackage.lastIndexOf(".") + 1);
  //                entryPackage = entryPackage.substring(0,
  //                entryPackage.lastIndexOf("." + entryClass));
  //
  //                JavaRegion javaRegion = new JavaRegion(entryPackage,
  //                entryClass, entryMethod, node.getBytecodeIndex()); boolean
  //                newRegion = true;
  //
  //                for(Map.Entry<JavaRegion, Set<String>> regionToFeature :
  //                relevantRegionToOptions.entrySet()) {
  ...
  //                            && relevantRegion.getStartBytecodeIndex() ==
  //                            javaRegion.getStartBytecodeIndex()) {
  ...
  //    }
  //
  //    public static List<String> getCriterionLabel(String file, String
  //    entryPoint, String criteriaFormat) throws IOException {
  //        List<String> criterionLabel = new ArrayList<>();
  //        SDG sdg = SDG.readFrom(file, new
  //        SecurityNode.SecurityNodeFactory()); Set<SDGNode> nodes =
  //        sdg.vertexSet(); List<SDGNode> entryPointNodes = new ArrayList<>();
  //
  //        for(SDGNode node : nodes) {
  ...
  //            if(node.getBytecodeMethod().contains(entryPoint) &&
  //            node.getLabel().contains(criteriaFormat)) {
