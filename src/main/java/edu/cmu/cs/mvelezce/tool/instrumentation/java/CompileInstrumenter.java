...
public class CompileInstrumenter extends BaseInstrumenter {
  ...
  public CompileInstrumenter(String programName, String srcDir,
                             String classDir) {
  ...
  public void compileFromSource() throws IOException, InterruptedException {
    ...
    try {
      ...
      Collection<File> m2Files =
          FileUtils.listFiles(m2Dir, new String[] {"jar"}, true);
      ...
      String[] command = new String[] {"javac",
                                       "-cp",
                                       cp.toString(),
                                       "-d",
                                       this.getClassDir(),
                                       "@" + this.getSrcDir() + "/sources.txt"};
  ...
  private void writeFilesToCompile() throws IOException, InterruptedException {
    ...
    BufferedWriter writer =
        new BufferedWriter(new FileWriter(this.getSrcDir() + "/sources.txt"));
    String string;
