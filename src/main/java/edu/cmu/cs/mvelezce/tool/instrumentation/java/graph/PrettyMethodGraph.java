...
public class PrettyMethodGraph extends MethodGraph {
  ...
  public void saveDotFile(String programName, String className,
                          String methodName) throws IOException {
    String dotString = this.toDotStringVerbose(methodName);
    File file =
        new File(BaseRegionInstrumenter.DIRECTORY + "/" + programName + "/" +
                 className + "/" + methodName + PrettyMethodGraph.DOT_DOT);
    file.getParentFile().mkdirs();

    PrintWriter writer = new PrintWriter(
        BaseRegionInstrumenter.DIRECTORY + "/" + programName + "/" + className +
  ...
  public void savePdfFile(String programName, String className,
                          String methodName)
      throws IOException, InterruptedException {
    List<String> commandList = new ArrayList<>();
    commandList.add("dot");
    commandList.add("-Tpdf");
    commandList.add(BaseRegionInstrumenter.DIRECTORY + "/" + programName + "/" +
                    className + "/" + methodName + PrettyMethodGraph.DOT_DOT);
    commandList.add("-o");
    commandList.add(BaseRegionInstrumenter.DIRECTORY + "/" + programName + "/" +
                    className + "/" + methodName + PrettyMethodGraph.DOT_PDF);
  ...
  public String toDotStringVerbose(String methodName) {
    ...
    for (MethodBlock methodBlock : blocks) {
      if (methodBlock == this.getEntryBlock() ||
          methodBlock == this.getExitBlock()) {
        continue;
      }
    ...
    for (PrettyMethodBlock prettyMethodBlock : prettyBlocks) {
      ...
      List<String> prettyInstructions =
          prettyMethodBlock.getPrettyInstructions();
    ...
    for (PrettyMethodBlock prettyBlock : prettyBlocks) {
      for (String prettyInstruction : prettyBlock.getPrettyInstructions()) {
        ...
        if ((prettyInstruction.contains("Regions") ||
             prettyInstruction.contains("RegionsCounter")) &&
            (prettyInstruction.contains("enter") ||
             prettyInstruction.contains("exit"))) {
    ...
    for (PrettyMethodBlock prettyBlock : instrumentedPrettyBlocks) {
      ...
      dotString.append(
          "[fontcolor=\"purple\", penwidth=3, color=\"purple\"];\n");
    }
