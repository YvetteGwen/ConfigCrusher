...
public abstract class RegionTransformer extends BaseMethodTransformer {
  ...
  private Map<SootMethod, Set<Set<String>>> sootMethodToOptionSet =
      new HashMap<>();
  ...
  private Map<MethodNode, LinkedHashMap<MethodBlock, JavaRegion>>
      methodsToBlocksDecisions = new HashMap<>();
  private Map<MethodNode, LinkedHashMap<MethodBlock, JavaRegion>>
      cachedMethodsToBlocksDecisions = new HashMap<>();
  private Map<JavaRegion, Set<Set<String>>> cachedRegionsToOptionSet =
      new HashMap<>();
  ...
  public RegionTransformer(String programName, String entryPoint,
                           String directory,
                           Map<JavaRegion, Set<Set<String>>> regionsToOptionSet)
      throws NoSuchMethodException, MalformedURLException,
             IllegalAccessException, InvocationTargetException {
    this(programName, entryPoint, new DefaultBaseClassTransformer(directory),
         regionsToOptionSet);
  }
  ...
  private void matchMethodsToOptions() {
    for (Map.Entry<JavaRegion, Set<Set<String>>> entry :
         this.regionsToOptionSet.entrySet()) {
      JavaRegion region = entry.getKey();
      String bytecodeSignature = region.getRegionPackage() + "." +
  ...
  private void calculateASMIndexes(Set<ClassNode> classNodes) {
    for (ClassNode classNode : classNodes) {
      ...
      Set<MethodNode> methodsToInstrument =
          this.getMethodsToInstrument(classNode);
  ...
  public void transformMethods(Set<ClassNode> classNodes) throws IOException {
    ...
    System.out.println("# of regions after optimizing: " +
                       this.regionsToOptionSet.size());
    System.out.println("");
  }
  ...
  private void cacheMethodsToBlockDecisions() {
    for (Map.Entry<MethodNode, LinkedHashMap<MethodBlock, JavaRegion>> entry :
         this.methodsToBlocksDecisions.entrySet()) {
      this.cachedMethodsToBlocksDecisions.put(
  ...
  private void setBlocksToDecisions(Set<ClassNode> classNodes) {
    for (ClassNode classNode : classNodes) {
      //            System.out.println("Setting blocks to decisions in class " +
      //            classNode.name);
      ...
      for (MethodNode methodNode : classNode.methods) {
        //                System.out.println("Setting blocks to decisions in
        //                method " + methodNode.name);
        List<JavaRegion> regionsInMethod = this.getRegionsInMethod(methodNode);
        LinkedHashMap<MethodBlock, JavaRegion> blocksToRegionSet =
            this.matchBlocksToRegion(methodNode, regionsInMethod);
  ...
  public void matchSootToASMMethods(Set<SootMethod> sootMethods,
    ...
    for (ClassNode classNode : classNodes) {
      ...
      for (MethodNode methodNode : methodNodes) {
        ...
        classPackageNode =
            classPackageNode.substring(0, classPackageNode.lastIndexOf("/"));
        classPackageNode = classPackageNode.replace("/", ".");

        String classNameNode = classNode.name;
        classNameNode =
            classNameNode.substring(classNameNode.lastIndexOf("/") + 1);

        String methodNameNode = methodNode.name + methodNode.desc;

        for (SootMethod sootMethod : sootMethods) {
          String classPackageSoot =
              sootMethod.getDeclaringClass().getPackageName();
          ...
          methodNameSoot = methodNameSoot
                               .substring(methodNameSoot.indexOf(" "),
                                          methodNameSoot.length() - 1)
                               .trim();

          if (classPackageNode.equals(classPackageSoot) &&
              classNameNode.equals(classNameSoot) &&
  ...
  private boolean propagateUpMethodsInClasses(Set<ClassNode> classNodes) {
    ...
    for (ClassNode classNode : classNodes) {
      ...
      for (MethodNode methodNode : methodsToInstrument) {
        ...
        updatedMethods =
            updatedMethods | this.propagateUpRegionsInMethod(methodNode);
      }
    }
  ...
  private boolean propagateUpAcrossMethods() {
    ...
    while (!worklist.isEmpty()) {
      ...
      //            System.out.println("Propagate regions across method " +
      //            a.getName());
  ...
  private List<SootMethod> propagateUpRegionInter(SootMethod method) {
    ...
    Collection<JavaRegion> regions =
        this.methodsToBlocksDecisions.get(methodNode).values();
    ...
    if (decision.isEmpty()) {
      throw new RuntimeException("The first decision in " + methodNode.name +
                                 " cannot be null");
    }
    ...
    for (Edge edge : edges) {
      ...
      MethodNode callerMethodNode =
          this.sootMethodToMethodNode.get(callerSootMethod);
      LinkedHashMap<MethodBlock, JavaRegion> callerBlocksToRegions =
          this.methodsToBlocksDecisions.get(callerMethodNode);
      ...
      if (!decision.containsAll(callerDecision) &&
          !decision.equals(callerDecision) &&
          !callerDecision.containsAll(decision)) {
        throw new RuntimeException(
            "Cannot push up " + decision + " from " + methodNode.name + " to " +
            callerMethodNode.name + " at " + callerBlock.getID() +
            " because it has decision " + callerDecision);
      }
      ...
      if (callerRegion == null) {
        String classPackage =
            callerSootMethod.getDeclaringClass().getPackageName();
        String className = callerSootMethod.getDeclaringClass().getShortName();
        String methodName = callerSootMethod.getBytecodeSignature();
        methodName =
            methodName
                .substring(methodName.indexOf(" "), methodName.length() - 1)
                .trim();
        index = callerMethodNode.instructions.indexOf(
            callerBlock.getInstructions().get(0));
        ...
        newRegion = new JavaRegion(callerRegion.getRegionPackage(),
                                   callerRegion.getRegionClass(),
                                   callerRegion.getRegionMethod(), index);
  ...
  private boolean canPropagateRegion(SootMethod method) {
    ...
    for (Edge edge : edges) {
      ...
      if (!decision.containsAll(callerDecision) &&
        ...
        System.out.println("Cannot push up " + decision + " from " +
                           methodNode.name + " to " + callerMethodNode.name +
                           " at " + callerBlock.getID() +
                           " because it has decision " + callerDecision);
        canPush = false;
        break;
      }
  ...
  private void instrument(Set<ClassNode> classNodes) throws IOException {
    for (ClassNode classNode : classNodes) {
      ...
      //            System.out.println("Setting where to instrument class " +
      //            classNode.name);

      for (MethodNode methodToInstrument : methodsToInstrument) {
        //                System.out.println("Setting where to instrument method
        //                " + methodToInstrument.name);
    ...
    for (ClassNode classNode : classNodes) {
      ...
      this.getClassTransformer().writeClass(
          classNode,
          this.getClassTransformer().getPath() + "/" + classNode.name);

      // Debugging
      TraceClassInspector classInspector =
          new TraceClassInspector(classNode.name);
      MethodTracer tracer = classInspector.visitClass();

      for (MethodNode methodNode : methodsToInstrument) {
        Printer printer = tracer.getPrinterForMethodSignature(methodNode.name +
                                                              methodNode.desc);
        PrettyMethodGraphBuilder prettyBuilder =
            new PrettyMethodGraphBuilder(methodNode, printer);
        PrettyMethodGraph prettyGraph = prettyBuilder.build();
        prettyGraph.saveDotFile(this.getProgramName(), classNode.name,
                                methodNode.name);

        try {
          prettyGraph.savePdfFile(this.getProgramName(), classNode.name,
                                  methodNode.name);
      ...
      for (MethodNode methodNode : classNode.methods) {
        ...
        for (JavaRegion r1 : regions) {
          ...
          if (!diff) {
            ...
            //                        multiple regions in " + classNode.name +
            //                        "-" + methodNode.name + " but all have the
            //                        same options");
            System.out.println("There are multiple regions in " +
  ...
  private boolean propagateUpRegionsInMethod(MethodNode methodNode) {
    ...
    LinkedHashMap<MethodBlock, JavaRegion> blocksToRegionSet =
        this.methodsToBlocksDecisions.get(methodNode);

    return this.propagateUpRegions(methodNode, blocksToRegionSet);
  }

  private boolean
  propagateUpRegions(MethodNode methodNode,
    ...
    while (!worklist.isEmpty()) {
      ...
      List<MethodBlock> blocks =
          this.propagateUpRegionsInMethod(methodNode, block);
    ...
    if (methodNode.name.equals("publish") &&
        this.methodNodeToClassNode.get(methodNode)
            .name.contains("CentralPublisher")) {
      System.out.println();
    }
  ...
  private List<MethodBlock> propagateUpRegionsInMethod(MethodNode methodNode,
                                                       MethodBlock block) {
    ...
    if (!(blockDecision.containsAll(idDecision) &&
          !blockDecision.equals(idDecision))) {
      //            this.debugBlockDecisions(methodNode);
      //                System.out.println("Cannot push up to id in " +
    ...
    if (block.getPredecessors().isEmpty()) {
      throw new RuntimeException("The predecessors cannot be empty " +
                                 block.getID());
    }
    ...
    for (MethodBlock pred : block.getPredecessors()) {
      ...
      if (!(blockDecision.containsAll(predDecision) ||
            blockDecision.equals(predDecision))) {
        if (pred.isCatchWithImplicitThrow()) {
          continue;
        }

        this.debugBlockDecisions(methodNode);
        throw new RuntimeException("Cannot push up decisions from " +
                                   block.getID() + " to " + pred.getID());

        ////                    System.out.println("Cannot push up to
        /// predecessor in " + methodNode.name + " " + bDecision + " -> " +
        /// aDecision);
        //                    continue;
      }

      JavaRegion newRegion = new JavaRegion(blockRegion.getRegionPackage(),
                                            blockRegion.getRegionClass(),
                                            blockRegion.getRegionMethod());
      int index;
  ...
  private void fillDownRegionsInMethod(MethodNode methodNode,
                                       MethodBlock block) {
    MethodGraph graph = this.getMethodGraph(methodNode);
    ...
    while (ipd != beta && blockDecision.equals(ipdDecision)) {
      ...
      //            if(temp == beta & ipd.getSuccessors().size() == 1 &&
      //            ipd.getSuccessors().iterator().next() == beta) {
      //                break;
      //            }
    ...
    for (MethodBlock reach : orderedReachables) {
      ...
      if (!(intersection.equals(blockDecision) ||
            intersection.equals(reachDecision)) &&
   ...
   * @return
   */
  private LinkedHashMap<MethodBlock, JavaRegion>
  matchBlocksToRegion(MethodNode methodNode, List<JavaRegion> regionsInMethod) {
    // Initialize
    ...
    Map<AbstractInsnNode, JavaRegion> instructionsToRegions =
        this.matchInstructionToRegion(methodNode, regionsInMethod);
   ...
   * @return
   */
  private Map<AbstractInsnNode, JavaRegion>
  matchInstructionToRegion(MethodNode methodNode,
    ...
    for (JavaRegion region : regionsInMethod) {
      instructionsToRegion.put(instructions.get(region.getStartBytecodeIndex()),
                               region);
    }
  ...
  private MethodGraph getMethodGraph(MethodNode methodNode) {
    ...
    if (graph == null) {
      DefaultMethodGraphBuilder builder =
          new DefaultMethodGraphBuilder(methodNode);
      graph = builder.build();
  ...
  private void debugBlockDecisions(MethodNode methodNode) {
    //        System.out.println("Debugging block decisions for " +
    //        methodNode.name);
    LinkedHashMap<MethodBlock, JavaRegion> blocksToRegions =
        this.methodsToBlocksDecisions.get(methodNode);
    ...
    StringBuilder dotString =
        new StringBuilder("digraph " + methodNode.name + " {\n");
  ...
  private void debugBlocksAndRegions(MethodNode methodNode) {
    ...
    if (blocksToDecisionCount != regionsInMethodCount) {
      throw new RuntimeException(
          "The number of regions in " + methodNode.name +
          " does not match in the "
  ...
  private void setStartAndEndBlocks(MethodNode methodNode) {
    ...
    LinkedHashMap<MethodBlock, JavaRegion> blocksToRegions =
        this.methodsToBlocksDecisions.get(methodNode);
    LinkedHashMap<MethodBlock, JavaRegion> cachedBlocksToRegions =
        this.cachedMethodsToBlocksDecisions.get(methodNode);

    for (Map.Entry<MethodBlock, JavaRegion> blockToRegion :
         blocksToRegions.entrySet()) {
      ...
      while (ipd != beta && (blockDecision.equals(ipdDecision) ||
                             blockDecision.containsAll(ipdDecision))) {
        MethodBlock temp = graph.getImmediatePostDominator(ipd);
      ...
      if (block == end) {
        throw new RuntimeException("Start and end equal");
                 ...
                 block.getSuccessors().iterator().next().equals(end)) {
        ends.add(block);
      ...
      for (MethodBlock e : ends) {
        ...
        if (ipd == beta & e.getSuccessors().size() == 1 &&
            ...
            (blockDecision.equals(eDecision) ||
             blockDecision.containsAll(eDecision))) {
          reachables.add(e);
        }
      }
  ...
  /**
   * TODO
   * Check if there is a region that does not have a connection to the exit
   * block. This happens when regions occur in handlers not connected to the
   * exit block
   */
  ...
  private void removeRegionsInMethod(MethodNode methodNode,
                                     Set<String> decision,
    ...
    for (MethodBlock block : reachables) {
      ...
      if (!(decision.equals(blockDecision) ||
            decision.containsAll(blockDecision))) {
        continue;
      }
   ...
   * @param reachables
   */
  private void removeRegionsInCallees(MethodNode methodNode,
                                      Set<String> decision,
                                      Set<MethodBlock> reachables) {
    ...
    while (!worklist.isEmpty()) {
      ...
      for (AbstractInsnNode inst : reach.getInstructions()) {
        ...
        // Optimization
        if (inst.getOpcode() < Opcodes.GETSTATIC ||
            inst.getOpcode() > Opcodes.MONITOREXIT) {
          continue;
        }
        ...
        for (Edge edge : calleeEdges) {
          ...
          MethodNode calleeMethodNode =
              this.sootMethodToMethodNode.get(calleeSootMethod);

          LinkedHashMap<MethodBlock, JavaRegion> calleeBlocksToRegions =
              this.methodsToBlocksDecisions.get(calleeMethodNode);
          ...
          for (Map.Entry<MethodBlock, JavaRegion> entry :
               calleeBlocksToRegions.entrySet()) {
            ...
            if (!(decision.equals(calleeDecision) ||
                  decision.containsAll(calleeDecision))) {
              MethodGraph calleegraph = this.getMethodGraph(calleeMethodNode);
              MethodBlock ipd =
                  calleegraph.getImmediatePostDominator(entry.getKey());
              Set<MethodBlock> rs =
                  calleegraph.getReachableBlocks(entry.getKey(), ipd);
              rs.remove(ipd);
              skip.addAll(rs);
              continue;
            }
  ...
  private boolean checkIfCanRemove(Set<String> decision,
                                   List<Edge> callerEdges) {
    ...
    while (!worklist.isEmpty()) {
      ...
      LinkedHashMap<MethodBlock, JavaRegion> blockDecisions =
          this.cachedMethodsToBlocksDecisions.get(method);
      ...
      if (callerDecision.isEmpty()) {
        ...
        for (Edge e : callers) {
          ...
          worklist.add(e);
                   ...
                   decision.containsAll(callerDecision))) {
        return false;
      }
    }
  ...
  private Unit getUnit(AbstractInsnNode inst, SootMethod sootMethod) {
    ...
    for (Unit unit : sootMethod.getActiveBody().getUnits()) {
      ...
      AbstractInsnNode asmInst =
          this.getASMInstruction(sootMethod, bytecodeIndex);
    ...
    if (match == null && inst instanceof MethodInsnNode) {
      throw new RuntimeException(
          "There has to be a instruction that calls a method");
    }

    //        if(match == null) {
    //            throw new RuntimeException("Could not find the instruction in
    //            this method");
    //        }
  ...
  public MethodBlock getBlockToEndInstrumentingBeforeIt(MethodGraph methodGraph,
    ...
    MethodBlock immediatePostDominator =
        methodGraph.getImmediatePostDominator(start);
    return immediatePostDominator;
  }

  public MethodBlock
  getBlockToStartInstrumentingBeforeIt(MethodGraph methodGraph,
                                       MethodBlock start) {
    MethodBlock id = methodGraph.getImmediateDominator(start);

    if (id != methodGraph.getEntryBlock() && id.getSuccessors().size() == 1 &&
        id.getSuccessors().contains(start)) {
      return id;
    }
  ...
  private List<String> getJavapResult(String classPackage, String className) {
    ...
    try {
      String[] command = new String[] {
          "javap", "-classpath", this.getClassTransformer().getPath(), "-p",
          "-c",    "-s",         classPackage + "." + className};
  ...
  private void calculateASMSIndexes(List<JavaRegion> regionsInMethod,
                                    MethodNode methodNode) {
    int methodStartIndex = this.getJavapStartIndex(methodNode);
    List<String> javapResult =
        this.getJavapResult(this.methodNodeToClassNode.get(methodNode));
    ...
    for (int i = methodStartIndex; i < javapResult.size(); i++) {
      ...
      String outputCommand =
          outputLine.substring(outputLine.indexOf(":") + 1).trim();
      ...
      String outputLineBytecodeIndexString =
          outputLine.substring(0, outputLine.indexOf(":")).trim();

      if (StringUtils.isNumeric(outputLineBytecodeIndexString)) {
        outputLineBytecodeIndex =
            Integer.valueOf(outputLineBytecodeIndexString);
      }
  ...
  private int getJavapStartIndex(MethodNode methodNode) {
    ...
    for (String outputLine : javapResult) {
      if (outputLine.equals(methodNameInJavap)) {
        ...
        javapDescriptor =
            javapDescriptor.substring(javapDescriptor.indexOf(" ")).trim();
    ...
    if (methodStartIndex == javapResult.size()) {
      throw new RuntimeException(
          "The start of the javap result cannot be the size of the result");
    }

    return methodStartIndex;
  }

  private AbstractInsnNode getASMInstruction(SootMethod callerMethod,
                                             int bytecodeIndex) {
    ...
    while (instructions.hasNext()) {
      ...
      if (instructionCounter == instructionNumber) {
        ...
        //                    throw new RuntimeException("The instruction has to
        //                    be a method call");
        //                }
  ...
  private List<Edge> getCallerEdges(SootMethod method) {
    ...
    while (!worklist.isEmpty()) {
      ...
      if (!src.method().getDeclaringClass().getPackageName().contains(
              this.rootPackage)) {
  ...
  private List<Edge> getCalleeEdges(SootMethod method) {
    ...
    while (!worklist.isEmpty()) {
      ...
      if (!tgt.getDeclaringClass().getPackageName().contains(
              this.rootPackage)) {
  ...
  private Set<SootMethod> getSystemMethods() {
    ...
    while (edges.hasNext()) {
      ...
      if (!src.getDeclaringClass().getPackageName().contains(
              this.rootPackage)) {
        continue;
      }
  ...
  private void initializeSoot() {
    String libPath =
        "/Library/Java/JavaVirtualMachines/jdk1.8.0_112.jdk/Contents/Home/jre/lib/rt.jar:/Library/Java/JavaVirtualMachines/jdk1.8.0_112.jdk/Contents/Home/jre/lib/jce.jar";
    ...
    Options.v().set_soot_classpath(
        appendClasspath(this.getClassTransformer().getPath(), libPath));
    ...
    if (!hasClasses) {
      throw new RuntimeException(
          "Only phantom classes loaded, skipping analysis...");
    }
  }
  ...
  private List<JavaRegion> getRegionsInMethod(SootMethod sootMethod) {
    ...
    methodName =
        methodName.substring(methodName.indexOf(" "), methodName.length() - 1)
            .trim();
    ...
    javaRegions.sort(
        Comparator.comparingInt(JavaRegion::getStartBytecodeIndex));
