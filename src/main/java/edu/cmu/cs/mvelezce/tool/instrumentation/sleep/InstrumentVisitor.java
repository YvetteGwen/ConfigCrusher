...
// TODO why do we pass the constraints? We are only instrumenting regions. Maybe
// change what regions are in the region selector so that we are not selecting
// regions that have the same constraints as outer region
...
public class InstrumentVisitor extends ReplacerVisitor {
  ...
  //        relevantRegionsToOptions; private
  //        Stack<Set<ConstantConfigurationExpression>> constraints;

  //        /**
  //         * Instantiate a {@code AddTimedVisitor}.
  //         */
  //        public AddTimedVisitor(Map<SleepRegion,
  ...
  public Statement visitProgram(Program program) {
    ...
    TimedProgram timedProgram =
        new TimedProgram(region.getRegionID().toString(), visitedProgram);
  ...
  public Statement visitIfStatement(IfStatement ifStatement) {
    ...
    if (region != null) {
      ...
      TimedStatement timedStatement = new TimedStatement(
          region.getRegionID().toString(), hold.getThenBlock());
  ...
  public Statement visitSleepStatement(SleepStatement sleepStatement) {
    ...
    if (region != null) {
      ...
      //                if(!this.constraints.contains(this.relevantRegionsToOptions.get(oldRegion)))
      //                {
      return new TimedStatement(region.getRegionID().toString(),
                                visitedSleepStatement);
      //                }
    }
