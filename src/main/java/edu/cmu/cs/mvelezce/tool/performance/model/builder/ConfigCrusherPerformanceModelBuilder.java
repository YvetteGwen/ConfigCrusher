 ...
 */
public class ConfigCrusherPerformanceModelBuilder
    extends BasePerformanceModelBuilder {

  public static final String DIRECTORY =
      Options.DIRECTORY + "/performance-model/java/programs/configcrusher";

  public ConfigCrusherPerformanceModelBuilder(
  ...
  // TODO write code that will know when multiple regions are executed in
  // separate
  @Override
  public PerformanceModel createModel() {
    // TODO add these method to the interface
    Map<Region, Map<Set<String>, Long>> regionsToPerformanceTable =
        this.processPerformance();
    long programTime = this.processProgramTime();
    PerformanceModel performanceModel =
        new PerformanceModel(programTime, regionsToPerformanceTable);
  ...
  private Map<Region, Map<Set<String>, Long>> averageMultipleExecutions(
      Map<Region, Map<Set<String>, Set<Long>>> regionsToPerformanceTable) {
    Map<Region, Map<Set<String>, Long>> result = new HashMap<>();

    for (Map.Entry<Region, Map<Set<String>, Set<Long>>>
             regionToPerformanceTable : regionsToPerformanceTable.entrySet()) {
  ...
  public Map<Region, Map<Set<String>, Long>> processPerformance() {
    ...
    for (Map.Entry<Region, Set<Set<String>>> regionToOptionSet :
      ...
      // TODO this might change if we decide to not have a set of set of options
      // for each region
    ...
    for (Map.Entry<Region, Map<Set<String>, Set<Long>>> entry :
         regionsToPerformanceTable.entrySet()) {
      Region region = entry.getKey();

      for (Map.Entry<Set<String>, Set<Long>> entry2 :
        ...
        if ((max - min) >= 1000000000) {
          ...
          System.out.println("Region: " + region.getRegionID() + " with conf " +
                             conf + " might be incorrect (" + maxD + " : " +
                             minD + ")");
    ...
    Map<Region, Map<Set<String>, Long>> regionsToAveragePerformanceTable =
        this.averageMultipleExecutions(regionsToPerformanceTable);
    return regionsToAveragePerformanceTable;
  }

  public Map<Region, Map<Set<String>, Double>> processStd() {
    Map<Region, Map<Set<String>, Set<Double>>> regionsToStdTable =
        new HashMap<>();
    ...
    for (Map.Entry<Region, Set<Set<String>>> regionToOptionSet :
      ...
      for (PerformanceEntryStatistic performanceEntry :
           this.getMeasuredPerformance()) {
        Set<String> configuration = performanceEntry.getConfiguration();
        Set<String> optionValueInPerfEntry = new HashSet<>(optionsInRegion);
        optionValueInPerfEntry.retainAll(configuration);
        ...
        for (Map.Entry<Region, Double> regionToProcessedPerformance :
             performanceEntry.getRegionsToProcessedStd().entrySet()) {
          Region performanceRegion = regionToProcessedPerformance.getKey();
    ...
    Map<Region, Map<Set<String>, Double>> regionsToAveragePerformanceTableStd =
        this.averageMultipleExecutionsStd(regionsToStdTable);
    return regionsToAveragePerformanceTableStd;
  }

  // TODO this is dangerous when regions are executed multiple times but they
  // had different times
  private Map<Region, Map<Set<String>, Double>> averageMultipleExecutionsStd(
      Map<Region, Map<Set<String>, Set<Double>>> regionsToStdTable) {
    Map<Region, Map<Set<String>, Double>> result = new HashMap<>();

    for (Map.Entry<Region, Map<Set<String>, Set<Double>>>
             regionToPerformanceTable : regionsToStdTable.entrySet()) {
