...
public class Compare {
  ...
  public static final String COMPARE_DIR =
      Options.DIRECTORY + "/comparison/java/programs";

  public static void compare(String programName, String approach1Dir,
                             String approach2Dir) throws IOException {
    FileInputStream fstream =
        new FileInputStream(approach1Dir + "/" + programName + Options.DOT_CSV);
    ...
    fstream =
        new FileInputStream(approach2Dir + "/" + programName + Options.DOT_CSV);
    ...
    //            throw new RuntimeException("The approach files do not have the
    //            same length");
    //        }
    ...
    while ((strLine = br.readLine()) != null && !strLine.equals(header)) {
      if (!strLine.isEmpty() && !strLine.contains("Average")) {
        ...
        String optionsString =
            strLine.substring(7, strLine.lastIndexOf('"') - 1);
    ...
    Set<PerformanceEntryStatistic> app1Stats =
        Compare.createEntries(programName, approach1Dir, configurations);
    Set<PerformanceEntryStatistic> app2Stats =
        Compare.createEntries(programName, approach2Dir, configurations);

    StringBuilder result = new StringBuilder();
    double se = 0;
    int entries = 0;
    result.append(
        "measured,configuration,ap1 mean,ap2 mean,absolute error,relative % error,squared error");
    result.append("\n");
    ...
    for (Set<String> configuration : configurations) {
      ...
      result.append('"');
      result.append(Boolean.valueOf(app1Entry.isMeasured()) &&
                    Boolean.valueOf(app2Entry.isMeasured()));
      result.append('"');
      result.append(",");
      result.append('"');
      ...
      //            double absoluteError = Math.abs(app2Entry.getMean() -
      //            app1Entry.getMean()); double relativeError = absoluteError /
      //            app2Entry.getMean() * 100.0; double squaredError =
      //            Math.pow(app2Entry.getMean() - app1Entry.getMean(), 2);
      //
    ...
    String outputFile = Compare.COMPARE_DIR + "/" + programName + "_" +
                        approach1 + "_" + approach2 + Options.DOT_CSV;
  ...
  private static Set<PerformanceEntryStatistic>
  createEntries(String programName, String approachDir,
                Set<Set<String>> configurations) throws IOException {
    Set<PerformanceEntryStatistic> entries = new HashSet<>();

    for (Set<String> configuration : configurations) {
      FileInputStream fstream = new FileInputStream(
          approachDir + "/" + programName + Options.DOT_CSV);
      DataInputStream in = new DataInputStream(fstream);
      BufferedReader br = new BufferedReader(new InputStreamReader(in));
      String strLine;
      String header = "";
      ...
      while ((strLine = br.readLine()) != null) {
        ...
        //                    perfStat = new PerformanceEntryStatistic(measured
        //                    + "", configuration, Double.valueOf(perfString),
        //                    0.0);
        //                }
        //                else if((strEntries.length - commaOffset) == 4) {
        //                    String stdString = strEntries[3 +
        //                    commaOffset].trim(); perfStat = new
        //                    PerformanceEntryStatistic(measured + "",
        //                    configuration, Double.valueOf(perfString),
        //                    Double.valueOf(stdString));
        //                }
        //                else {
        //                    throw new RuntimeException("Could not parse the
        //                    file");
        //                }
        //
